...
本文档列出**所有后端 HTTP API 端点**：请求格式、参数、响应、错误码、限流规则。
如果你要：

- 写前端代码调用后端 → 直接对照本文档
- 写脚本自动化操作 → 看每个端点的 curl 示例
- 调试接口问题 → 看错误码表（§11）

**实时 API 文档**：开发环境访问 `http://localhost:8000/docs`（Swagger UI）或 `/redoc` 查看 OpenAPI 自动生成的可交互文档（生产环境会被关闭）。
