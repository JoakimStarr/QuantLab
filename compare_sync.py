...
# 新机制预期的日志关键词 → 说明（关键词须与代码中 logger 字符串精确一致）
EVIDENCE_KEYS = [
    ("动态成分缓存命中", "缓存复用成功（首次运行无缓存属正常，次日才会命中）"),
    ("熔断", "熔断机制触发（因连接衰减触发为正常；数据正常时为误触发）"),
    ("akshare 兜底", "指数 baostock 失败后走 akshare"),
    ("跳过外盘/宏观重广播", "日历未变化时跳过重广播（日历有推进时不出现属正常）"),
    ("宏观字段无变化", "指纹判重跳过广播（宏观有新数据时不出现属正常）"),
    ("days_failed", "ETF 失败天数如实上报（阶段3结果 dict）"),
    ("warnings=['", "完成日志携带非空 warnings"),
]

KEYS = ["sync_worker", "阶段", "同步完成", "交易日数", "已按新日历补齐",
