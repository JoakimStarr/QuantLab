      ...
      <div class="factor-stats__bars">
        <div class="factor-stats__bar" v-for="item in sourceBars" :key="item.key">
          ...
          <div class="factor-stats__bar-track">
            <div class="factor-stats__bar-fill" :class="`badge--${item.badge}`" :style="{ width: item.percent + '%' }" />
          </div>
        </div>
      </div>
    </div>
...
.factor-stats { display: flex; flex-direction: column; gap: 16px; }
.factor-stats__total {
  display: flex; align-items: baseline; gap: 12px;
  padding-bottom: 12px; border-bottom: 1px solid var(--border-light);
}
.factor-stats__total-label { font-size: 14px; color: var(--text-tertiary); }
.factor-stats__total-value {
  font-size: 28px; font-weight: 600; color: var(--text-primary);
  font-variant-numeric: tabular-nums;
}
.factor-stats__bars { display: flex; flex-direction: column; gap: 10px; }
.factor-stats__bar { display: flex; flex-direction: column; gap: 4px; }
...
.factor-stats__bar-fill { height: 100%; border-radius: 3px; transition: width .3s; }
.badge {
...
.factor-stats__bar-fill.badge--info { background: var(--info); }
.factor-stats__bar-fill.badge--danger { background: var(--danger); }
