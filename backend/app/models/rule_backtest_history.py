...
每次运行 /strategy-library/backtest 自动落库一条记录（配置 + 结果快照），
前端策略库页面下方展示历史列表，支持回看详情、一键重跑、删除。
"""
...
class RuleBacktestHistory(Base):
    """策略库规则回测历史（自动保存，配置 + 指标 + 净值/成交快照）"""
    __tablename__ = "rule_backtest_history"

    id = Column(Integer, primary_key=True, autoincrement=True)
    # 模板快照（模板可增删改，历史需保留当时的名称/类别）
    ...
    kind = Column(String(8), nullable=True)  # single / pairs

    # 配置快照
    params = Column(Text, nullable=True, comment="配置参数 JSON")
    symbols = Column(Text, nullable=False, comment="标的列表 JSON，如 ['sh600000']")
    ...
    __table_args__ = (Index("idx_rbh_created_at", "created_at"),)
