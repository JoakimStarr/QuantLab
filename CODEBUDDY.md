...
- `quant/` — qlib integration: `factor_eval.py` (IC/RankIC/ICIR/turnover evaluation, handles `AutoML(...)` expressions via trained bundles), `backtest_engine.py` / `qlib_backtest.py` / `vbt_backtest.py` (backtest backends), `portfolio*.py`, `walk_forward.py`, `qlib_init.py` (provider_uri init), `rule_backtest.py` (策略库 v1 规则型策略：布林带/双均线/RSI/均线排列/MACD/动量/配对，信号→vbt.Portfolio.from_signals；**交易方向取自 `orders.records_readable` 的 `Side` 列**，Size 恒为正，别用 Size 正负判断买卖；`run_rule_backtest` 支持 `initial_capital` 初始资金参数，默认 1000 万，结果自动落 `rule_backtest_history` 历史). `monte_carlo.py` — 蒙特卡罗分析：① 回测指标 bootstrap 置信区间（`arch` StationaryBootstrap 重采样日收益 → Sharpe/回撤/胜率等 7 指标分布与 90% CI；端点 `POST /strategies/backtest-results/{id}/monte-carlo`，从 nav_curve 重建收益实时算、进程内 LRU 缓存、不改库）；② 因子 IC 置换检验（`permutation_ic_test` 逐日截面打乱因子值构建 IC 零分布 → 非参数 p-value，接入 `deep_analyze_factor` summary 的 `perm_pvalue`，深度分析页展示，不拦截挖掘）。
...
- `data/` — data acquisition & integrity. **`sync_worker.py` is the execution backbone**: every sync/repair runs as an independent subprocess (`spawn_sync_worker(kind, universe, ...)`, `start_new_session=True`, one process per `kind`) that reports progress via `data/sync_progress.json` (web process reads it; `sync_progress.py` + `sync_lock.py` provide a flock "crawl lock" that **serializes all baostock crawlers** — backfill/eod/indices/repair/full; akshare/eastmoney jobs skip it). Kinds: `backfill` (A股 baostock), `eod` (incremental, baostock/akshare via `source`), `repair` (一键补齐), `indices`, `fundamental`, `macro` (宏观同步, eastmoney/akshare), `full` (一键全同步, see below). Key modules:
  - `baostock_backfill.py` — primary A股 sync (triggered via `POST /quant/data/sync-full` stage 1 or repair's `include_baostock`; manual only): newest→oldest, one `query_daily_history_k_AStock` per trading day (whole market), writes qlib bin + PG `stock_daily`/`stock_basic`/`stock_industry`/`trade_calendar`, builds `instruments/*.txt`. Idempotent (`ON CONFLICT DO NOTHING`). 全市场接口不支持复权 → `_build_out_df` 先写原始价，随后统一经 `data_adjusted.apply_hfq_transform` 物化为**后复权（hfq）**并把真实累计复权因子写入 `factor`（全量重建用 `backend/scripts/rebuild_adjusted_bins.py`，成功路径也会重建 `all/csiall` instruments）。**`_rebuild_dynamic_instruments` 拉指数成分失败返回空集时不会覆盖原 csi300.txt/csi500.txt**（否则默认 universe 的挖掘/回测读到空池"无数据"）。
  - `eod_incremental.py` — bin read/write/merge helpers (`_sync_stock_bin`, `_write_calendar`, `_compute_tradable`, **`_pad_bins_to_calendar`**) + `incremental_sync_eod` (EOD for recent days). **候选交易日以「stock_daily 是否已有数据」判断（`_covered_trade_dates`），不是"是否在 day.txt 日历里"**——日历会被 padding 到当天，按日历判断会让当天数据永不落库。
...
- **`factor` 是真实累计后复权因子（hfq 口径，锚定数据起点）**，不是 baostock 字段，更不是恒 1.0（历史遗留：2026-09 前曾是不复权+1.0，已全量重建）。当 `BIN_FIELDS` 变更时，existing bins must be regenerated (e.g., a fresh backfill/repair) or the missing-field check flags every stock.
