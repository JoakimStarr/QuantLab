...
- **Logging** (`app/core/logging_config.py`): 统一 structlog JSON，web 进程与 sync worker 共用 `setup_logging()`。3 个文件：`quantlab.log`（web 全量 INFO+）、`error.log`（web WARNING+，备份保留 15 天）、`sync.log`（全部 worker，行内 `worker_kind` 字段区分任务类型）。`LockedRotatingFileHandler`（fcntl）保证 worker 跨进程写 `sync.log` 安全。**消噪**：`uvicorn.access`→WARNING、`asgi_correlation_id`→ERROR（曾淹没 error.log）、`apscheduler`→WARNING；uvicorn 启动日志已落盘。`PUT /logs/level` 运行时调级（DEBUG 排查：切 DEBUG→复现→看 quantlab.log→切回 INFO）。审计事件走统一管道（`logger="audit"`，登录/登出/挖掘/回测提交）。清理模式只匹配数字后缀备份（`quantlab.log.[0-9]*` 等），不会误删 `.lock` 锁文件。
